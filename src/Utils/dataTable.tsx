...
interface IgenerateCustomIdProps {
  id: number;
  prefix: string;
  totalLength?: number;
}
export const OnTextRender: React.FC<ITextRenderProps> = ({
  text,
  isBold = false,
}) => {
  return (
...
export const GenerateCustomId: React.FC<IgenerateCustomIdProps> = ({
  id,
  prefix,
  totalLength = 4,
}): any => {
  const paddedId = id.toString().padStart(totalLength, "0");
  return <OnTextRender text={`${prefix}${paddedId}`} isBold={true} />;
};
